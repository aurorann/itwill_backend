...
--문1) 주소별 인원수를 조회하시오
select addr, count(*) as cnt --집계함수, 칼럼명 임시 부여
from sungjuk
group by addr;
...
--문3) 총점(tot), 평균(aver)을 구하시오
update sungjuk set tot=kor+eng+mat, aver=(kor+eng+mat)/3;
commit;
select * from sungjuk;

--문4) 평균(aver)이 80점 이상 행을 대상으로 주소별 인원수를 인원수순으로 조회하시오
...
select addr, count(*)-- 4번째 해석
from sungjuk         -- 1번째 해석
where aver>=80       -- 2번째 해석
group by addr        -- 3번째 해석
order by count(*);   -- 5번째 해석
//////////////////////////////////////////////////////////////////////////
