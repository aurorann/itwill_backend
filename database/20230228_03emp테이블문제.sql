...
create table emp(
  ...
  ,job        varchar2(10)              --직급(사원,주임,과장,부장,대표)
  ,mgr        number(4)                 --매니저정보
  ,hiredate   date                      --입사일
...
update (select * from emp where deptno=20)
set comm=(sal*0.01);
...
select ename, sal, nvl(comm,0), round(sal*12+nvl(comm,0),0) as 연봉
from emp;
...
emp 테이블에서 급여가 200보다 많고 300보다 적지 않은 급여를 오름차순으로 정렬하고 사원번호 조회

문30) 다음의 SQL문을 분석하시오
      select empno, sal 
      from emp
      where not sal>200 and sal<300 --200이하이면서 300미만 행 조회
      order by sal;
      
emp 테이블에서 급여가 200보다는 많지 않고 300보다는 적은 급여를 오름차순으로 정렬하고 사원번호 조회
...
select deptno, sum(sal) -- 4번째 실행
from emp                -- 1번째 실행
group by deptno         -- 2번째 실행
having sum(sal)>=1500   -- 3번째 실행
order by sum(sal);      -- 5번째 실행
...
select hiredate, to_char(hiredate,'mm') from emp; -- 입사일에서 월 가져오기
select hiredate, to_char(hiredate,'mm') as 월 from emp; -- 입사일에서 월 가져오기
select hiredate, to_char(hiredate,'dd') as 일 from emp; -- 입사일에서 일 가져오기
select hiredate, to_char(hiredate,'yy') as 입사년도 from emp; -- 입사일에서 연 가져오기
...
select ename, sal from emp where empno=7654; -- 사원번호 7654 급여 400
...
select deptno, max(sal+nvl(comm,0)) 최대값, min(sal+nvl(comm,0)) 최소값, avg(sal+nvl(comm,0)) 평균값
from emp
group by deptno
order by deptno;
...
--경은 코드
select sum(sal), nvl(sum(comm),0) from emp group by deptno; -- 부서별 급여, 커미션
select sum(sal)+nvl(sum(comm),0) from emp group by deptno; -- 부서별 급여+커미션

select deptno, max (select sum(sal)+nvl(sum(comm),0) from emp group by deptno)
             , min (select sum(sal)+nvl(sum(comm),0) from emp group by deptno)
             , avg (select sum(sal)+nvl(sum(comm),0) from emp group by deptno)
from emp
group by deptno;
...
-- 근속일 : 현재날짜 - 입사일 2023-02-28 - 2000-07-12
...
-- (현재날짜-입사일)/365 -> 소수점 버림 -> 칼럼명 변경 -> 근속년수 내림차순
...
select trunc((sysdate - hiredate)/365) from emp where ename='손흥민';--21
...
--경은코드
select ename, trunc((sysdate - hiredate)/365) from emp order by hiredate;
select trunc((sysdate - hiredate)/365) from emp where ename='손흥민' order by hiredate;

select ename, trunc((sysdate - hiredate)/365) as 근속년수
from emp
where (select trunc((sysdate - hiredate)/365) from emp where ename='손흥민')=(trunc((sysdate - hiredate)/365));
--중복쿼리에 order by가 들어가면 에러가 난다

문43) 입사한지 만 20년 이상된 사람에 한해 현재연봉에서 10% 인상시켰을 때 
      사번, 이름, 입사일, 현재연봉, 인상후연봉, 인상된금액으로 고액연봉순으로 조회하시오
      연봉구하는 식 : 급여(sal)*12개월+보너스(comm)
      
--입사한지 만 15년 이상 조회
select ename, trunc((sysdate-hiredate)/365) 근속년도
from emp
...
--현재 연봉 구하기(단, 커미션이 없으면 0으로 바꾸어서 계산)
...
--짝수년도 추출하기
select job, sal, hiredate, extract(YEAR from hiredate) as 입사년도
from emp
