...
3) 2결과 국어평균값이 50이상 조회하기
select addr, round(avg(kor),0)
from sungjuk
group by addr
having round(avg(kor),0)>=50;

4) 3결과 국어평균값을 내림차순으로 정렬
select addr, round(avg(kor),0)
from sungjuk
group by addr
having round(avg(kor),0)>=50
...
5) 4결과 국어평균값 칼럼명을 avg_kor로 변경하기
...
select *
from sungjuk
where aver>=70; --where로 대상 행을 구하기

3) 2결과에서 주소별 인원수를 구하시오
select addr, count(*)
from sungjuk
where aver>=70
group by addr;

4) 3결과에서 그 인원수가 2미만 행 조회
select addr, count(*)
from sungjuk
where aver>=70
group by addr
...
group by addr
having count(*)<2
order by cnt; -- order by count(*) 써도 가능
