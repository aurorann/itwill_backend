...
select lower('Hello World') from dual;        -- 문자 모두 소문자로 변환
select upper('Hello World') from dual;        -- 문자 모두 대문자로 변환
select length('Hello World') from dual;       -- 글자 갯수
select substr('Hello World', 1, 5) from dual; -- 문자열 자르기(첫번째부터 5개)'Hello'

select instr('HelloWorld', 'W') from dual;    -- 특정문자 위치
select lpad('SKY', 5, '*') from dual;         -- 5칸내에서 출력하고 왼쪽 빈칸은 *으로 채움
select rpad('SKY', 5, '*') from dual;         -- 5칸내에서 출력하고 오른쪽 빈캄은 *으로 채움
...
select mod(5,3) from dual;              -- 나머지값 함수
select ceil(12.4) from dual;            -- 올림함수
select trunc(13.56,1) from dual;        -- 내림함수 trunc(n. m)- 해당 숫자 n에서 m자리까지 버림하는 함수

select avg(kor) from sungjuk;           --66.363636
select ceil(avg(kor)) from sungjuk;     --67
select trunc(avg(kor),2) from sungjuk;  --66.36
select round(avg(kor),3) from sungjuk;  --66.364 round(n, m) - 해당 숫자 n에 m자리까지 반올림하는 함수
